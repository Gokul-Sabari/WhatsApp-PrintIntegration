...
useEffect(() => {
  if (loading || error || !salesOrder || hasDownloaded.current) return;
  ...
  if (!loading && !error && salesOrder && !hasDownloaded.current) {
    ...
    const attemptDownload = () => {
      if (printRef.current) {
        ...
        const delay = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent) ? 2000 : 1000;
  ...
  if (!loading && !error && salesOrder && !hasDownloaded.current) {
    ...
    const attemptDownload = () => {
      ...
      if (printRef.current) {
        ...
        // Mobile needs more time for rendering
        ...
        const timer = setTimeout(attemptDownload, 200);
        return () => clearTimeout(timer);
      }
    };
